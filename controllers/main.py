...
class Main(QMainWindow, f_PyStruct):
        ...
        if contaPestanas > 0:
            ...
            if nomPestana != 'PyStruct 2.0':
                    ...
                    for row in range(globals()["tabla_"+str(nomPestana)].rowCount()):
                        for column in range(globals()["tabla_"+str(nomPestana)].columnCount()):
                            ...
                            if globals()["tabla_"+str(nomPestana)].item(row, column) is not None:
                                ...
                                valor = float(
                                    globals()["tabla_"+str(nomPestana)].item(row, column).text())
                                # Formato de guardado
                ...
                mensajes.mensajeError(
                    'Error de Exportación', 'La pestaña PyStruct no puede ser exportada')
        else:
            # Mensaje de error por no existir pestañas para exportar
            mensajes.mensajeError('Error Exportación',
                                  'No hay ninguna pestaña para exportar')
    ...
    def graficarSeleccion(self):
        ...
        if nomPestana != 'PyStruct 2.0':
            ...
            rangoSeleccionado = globals(
            )["tabla_"+str(nomPestana)].selectedRanges()
                ...
                if (maxcol-mincol) > 0:
                    ...
                    if seleccionTipoGrafica != 'Tipo':
                            ...
                            if validarContenido == True:
                                ...
                                if seleccionTipoGrafica == 'Dispersión':
                                    ...
                                    if rescol == 3:
                                        ...
                                        for i in range(minrow, maxrow+1):
                                            lista.append(
                                                float(globals()["tabla_"+str(nomPestana)].item(i, mincol).text()))
                                            lista.append(
                                                float(globals()["tabla_"+str(nomPestana)].item(i, mincol+1).text()))
                                            lista.append(
                                                float(globals()["tabla_"+str(nomPestana)].item(i, maxcol).text()))
                                            lista2.append(lista)
                                            lista = []
                                        # Crear dataframe con los datos capturados
                                        dataset = pd.DataFrame(lista2)
                                        # Crear instancia del módulo GraficaDispersion e inicializarlo
                                        window = GraficaDispersion(
                                            dataset, rescol, self)
                                        window.show()
                                    # Condicional para dispersión 2D
                                        ...
                                        mensajes.mensajeError(
                                            'Error de Parámetros', 'Debe seleccionar entre 2 y 3 columnas para este tipo de gráfica')
                                # Condicional para cuando el tipo de gráfica es Lineal
                                    ...
                                    if rescol == 2:
                                        ...
                                        mensajes.mensajeError(
                                            'Error de Parámetros', 'Debe seleccionar exactamente dos columnas para este tipo de gráfica')
                            else:
                        ...
                        mensajes.mensajeAlerta(
                            'Alerta', 'Debe seleccionar un tipo de gráfica')
                else:
                    # Mensaje de alerta por si el usuario selecciona solo una columna
                    mensajes.mensajeAlerta(
                        'Alerta', 'Seleccione mas de una columna')
            else:
                # Mensaje de error por si no se cuentan con datos seleccionados
                mensajes.mensajeError(
                    'Error de Datos', 'No se cuenta con datos seleccionados para graficar')
        else:
            # Mensajes de error en el caso de que quieran gráficar PyStruct
            mensajes.mensajeError(
                'Error de Datos', 'La pestaña PyStruct no puede ser graficada, debido a su contenido')
            ...
            for i in range(filas):
                for j in range(columnas):
                    ...
                    globals()["tabla_New_Tab_"+str(contaNewPestanas)
                              ].setItem(i, j, QTableWidgetItem(str(dato)))
        else:
            ...
            if rangoSeleccionado != 0:
                ...
                for i in range(minrow, maxrow+1):
                    for j in range(mincol, maxcol+1):
                        if globals()["tabla_"+str(nomPestana)].item(i, j) != None:
                            lista.append(
                                float(globals()["tabla_"+str(nomPestana)].item(i, j).text()))
                        else:
                            lista.append(' ')
                    lista2.append(lista)
                    lista = []
                ...
                mensajes.mensajeError(
                    'Error de Datos', 'No se cuenta con datos seleccionados para copiar')
        else:
            # Mensaje de error, al intentar copiar la pestaña PyStuct
            mensajes.mensajeError(
                'Error de Parámetros', 'La pestaña PyStruct no puede ser copiada, debido a su contenido')
        ...
        if nomPestana != 'PyStruct 2.0':
            ...
            if rangoSeleccionado != []:
                ...
                for i in range(filas):
                    for j in range(columnas):
                        dato = dataset.iloc[i, j]
                        globals()["tabla_"+str(nomPestana)].setItem(minrow,
                                                                    mincol, QTableWidgetItem(str(dato)))
                        mincol = mincol + 1
                    minrow = minrow + 1
                ...
                mensajes.mensajeError(
                    'Error', 'Debe indicar la posición inicial para pegar el contenido')
        else:
            # Mensaje de error, para el momento en que quieran pegar en PyStruct
            mensajes.mensajeError(
                'Error de Parámetros', 'En la pestaña PyStruct no se puede pegar contenido')
    ...
    def cortarSeleccion(self):
        ...
        if nomPestana != 'PyStruct 2.0':
            ...
            if rangoSeleccionado != 0:
                ...
                for i in range(minrow, maxrow+1):
                    for j in range(mincol, maxcol+1):
                        globals()["tabla_"+str(nomPestana)
                                  ].item(i, j).setText('')

            else:
                # Mensaje de error, para cuando no se tengan datos seeccionados
                mensajes.mensajeError(
                    'Error de Datos', 'No se cuenta con datos seleccionados para ejecutar cortar')
        else:
            # Mensaje de error, para el momento en que quieran cortar PyStruct
            mensajes.mensajeError(
                'Error de Parámetros', 'La pestaña PyStruct no puede ser cortada, debido a su contenido')
    ...
    def documentacionApp(self, QModelidx):
        ...
        os.startfile(
            ".\\Documentacion\\Solicitud Academica original 4253_1.pdf")
    ...
    def estiloCentrar(self):
        ...
        if nomPestana != 'PyStruct 2.0':
            ...
            if rangoSeleccionado != []:
                ...
                for i in range(minrow, maxrow+1):
                    for j in range(mincol, maxcol+1):
                        globals()["tabla_"+str(nomPestana)].item(i,
                                                                 j).setTextAlignment(Qt.AlignCenter)
            else:
                # Mensaje de error, en caso de no seleccionar celdas
                mensajes.mensajeError(
                    'Error de estilos', 'No se cuenta con datos seleccionados para aplicar el estilo')
        else:
            # Mensaje de error, en el caso de querer dar estilo a la pestaña PyStruct
            mensajes.mensajeError(
                'Error de estilos', 'A la pestaña PyStruct no se le puede aplicar estilos')
        ...
        if nomPestana != 'PyStruct 2.0':
            ...
            if rangoSeleccionado != []:
                ...
                for i in range(minrow, maxrow+1):
                    for j in range(mincol, maxcol+1):
                        globals()["tabla_"+str(nomPestana)].item(i,
                                                                 j).setTextAlignment(Qt.AlignLeft)
            else:
                # Mensaje de error, en caso de no seleccionar celdas
                mensajes.mensajeError(
                    'Error de estilos', 'No se cuenta con datos seleccionados para aplicar el estilo')
    ...
    def estiloNegrilla(self):
        ...
        if nomPestana != 'PyStruct 2.0':
            ...
            if rangoSeleccionado != []:
                ...
                for i in range(minrow, maxrow+1):
                    for j in range(mincol, maxcol+1):
                        font = globals()["tabla_"+str(nomPestana)
                                         ].item(i, j).font()
                        font.setBold(True)
                        globals()["tabla_"+str(nomPestana)
        ...
        if nomPestana != 'PyStruct 2.0':
            ...
            if rangoSeleccionado != []:
                ...
                for i in range(minrow, maxrow+1):
                    for j in range(mincol, maxcol+1):
                        globals()["tabla_"+str(nomPestana)].item(i,
                                                                 j).setForeground(QColor(color.name()))
            else:
                # Mensaje de error, en caso de no seleccionar celdas
                mensajes.mensajeError(
                    'Error de estilos', 'No se cuenta con datos seleccionados para aplicar el estilo')
